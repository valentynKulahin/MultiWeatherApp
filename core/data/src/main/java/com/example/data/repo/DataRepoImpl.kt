...
@Singleton
internal class DataRepoImpl @Inject constructor(
    ...
    override suspend fun getTopNews(country: String): NewsDataModel {
        when (networkMonitor.networkStatus.first()) {
            is NetworkStatus.Connected -> {
                return when (val result = newsDataRemoteRepo.getTopNews(country = country)) {
    ...
    override suspend fun getSearchingCountriesList(country: String): List<CountryItemDataModel> {
        when (networkMonitor.networkStatus.first()) {
            is NetworkStatus.Connected -> {
                return when (val result =
                    weatherDataRemoteRepo.getSearchingCountriesList(country = country)) {
                    is ApiResult.ApiSuccess -> {
                        val searchingList = result.data as List<CountryItemDataModel>
                        return searchingList
                    }

                    is ApiResult.ApiException -> {
                        emptyList<CountryItemDataModel>()
                    }
        ...
        return emptyList<CountryItemDataModel>()
    }

    override suspend fun addCountryToFavourite(countryItemDataModel: CountryItemDataModel) {
        weatherDataLocalRepo.addCountryToFavourite(countryItemDataModel = countryItemDataModel)
    }

    override fun getFavouriteCountries(): Flow<List<CountryItemDataModel>> {
        return weatherDataLocalRepo.getFavouriteCountries()
    }

    override suspend fun deleteCountryFromFavourite(countryItemDataModel: CountryItemDataModel) {
        weatherDataLocalRepo.deleteCountryFromFavourite(countryItemDataModel = countryItemDataModel)
    }
